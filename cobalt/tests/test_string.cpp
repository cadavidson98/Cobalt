...
TEST(CobaltCoreStringTests, TestSplit) {
    {
        ...
        std::vector<std::string> strings = cobalt::core::split<std::string>(testString, ' ');
    ...
    }
    {
        ...
        std::vector<std::string> groups = cobalt::core::split<std::string>(testString, ' ');
        EXPECT_EQ(groups.size(), 3);
        EXPECT_EQ(groups[0], "1/2/3");
        EXPECT_EQ(groups[1], "4//5");
        EXPECT_EQ(groups[2], "6/7/");
        {
            std::vector<uint32_t> ints = cobalt::core::split<uint32_t>(groups[0], '/');
        ...
        {
            std::vector<uint32_t> ints = cobalt::core::split<uint32_t>(groups[1], '/');
            EXPECT_EQ(ints.size(), 2);
            EXPECT_EQ(ints[0], 4);
            EXPECT_EQ(ints[1], 5);
        }
        {
            std::vector<uint32_t> ints = cobalt::core::split<uint32_t>(groups[2], '/');
            EXPECT_EQ(ints.size(), 2);
            EXPECT_EQ(ints[0], 6);
            EXPECT_EQ(ints[1], 7);
        }
    }
    {
        ...
        std::vector<float> floats = cobalt::core::split<float>(testString, ',');
...
TEST(CobaltCoreStringTests, TestFileExtension) {
    {
        ...
        std::string extension = cobalt::core::fileExtension(objFile);
        EXPECT_EQ(extension, "obj");
    }
    {
        std::string_view pngFile = "/home/users/files/image.png";
        std::string extension = cobalt::core::fileExtension(pngFile);
        EXPECT_EQ(extension, "png");
    }
    {
        std::string_view tarFile = "/tmp/data.tar.gz";
        std::string extension = cobalt::core::fileExtension(tarFile);
        EXPECT_EQ(extension, "gz");
    }
    {
        std::string_view noFile = "test_no";
        std::string extension = cobalt::core::fileExtension(noFile);
        EXPECT_EQ(extension, "");
    }
}
