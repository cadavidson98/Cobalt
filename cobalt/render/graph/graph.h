...
public:
    ...
    static std::shared_ptr<CoGraph> createWithTypedRoot();

    template<NodeType nodeType, typename literal>
    void declareInput(std::string_view name, literal &&fallback);

private:
    std::shared_ptr<Node> _base = {};
    std::unordered_map<std::string, std::shared_ptr<Node>> _inputNodes = {};

    CoGraph() = default;
    CoGraph(std::shared_ptr<Node> base);
};
