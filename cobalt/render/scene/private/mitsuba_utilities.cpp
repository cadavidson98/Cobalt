...
std::optional<MitsubaMaterial> loadMaterial(xmlNodePtr materialNode, xmlXPathContextPtr context, const MitsubaSchema &schema) {
...
std::optional<MitsubaScene> readMitsuba(
    ...
    if (!sensorObject || !sensorObject->nodesetval || !meshesObject ||
        !meshesObject->nodesetval || !emittersObject || !emittersObject->nodesetval) {
